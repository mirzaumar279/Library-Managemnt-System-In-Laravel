    ...
    <table id="customers">
        ...
        <tbody>
            ...
            @foreach ($bookArr as $book)
                <tr>
                    <td>{!! $generate->getBarcode("$book->id", $generate::TYPE_CODE_128) !!}
                        ID-{{ $book->id }}
                    </td>
                    <td>{{ $book->title }}</td>

                    <td><img class="im" src="{{ asset('uploads/bookimage/' . $book->image) }}" width="100px"
                    ...
                    <td>{{ $book->quantity }}</td>
                    <td>
                        <a href="bookedit/{{ $book->id }}" class="btn btn-outline-success">Edit</a>
                        &nbsp;
                        <a href="bookdelete/{{ $book->id }}" class="btn btn-outline-danger">Delete</a>
                        &nbsp;
                        <a href="bookdetail/{{ $book->id }}" class="btn btn-outline-primary">Details</a>
                    </td>
                </tr>
            @endforeach
            <tr>
                <td class="kk" colspan="9"><a href="bookadd">Add Books</a></td>
            </tr>
        </tbody>
    </table>
